    ...
    for S in S_list:
        for phase in oper_dict:
            ...
            for case_idx, case in enumerate(cases):
                ...
                planeB_maxdelta = asb.Airplane(
                    name="Initial Aircraft",
                    xyz_ref=[MAC / 4, 0, 0],
                    wings=[wingB_maxdelta, vertical_tail, horizontal_tail],
                    # fuselages=[fuselage]
                )
                ...
                if phase.lower() == "cruise":
                    ...
                    plane = [planeB if phase == "landing" else planeB_maxdelta][0]
                    jvl_plane = JVL(
                        airplane=plane,
                        op_point=asb.OperatingPoint(
                            velocity=velocity,
                ...
                jvl_plane.write_jvl(
                    f"./JVL_writer/sref_trades/run_outputs/geoms/{config}",
            ...
            save_results(
                results,
                f"./JVL_writer/sref_trades/run_outputs/coeff_results/Sref-{S}/{phase}.pkl",
            )
